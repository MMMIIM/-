...
test('Enterprise Retrieval pgvector Top-K、隔离、失效与审计闭环',async()=>{
  ...
  try{
    ...
    const firstCount=Number((await pool.query(`SELECT count(*) FROM material_chunk_embeddings WHERE chunk_id=ANY($1::text[])`,[[relevant.chunk.chunk_id,history.chunk.chunk_id,unrelated.chunk.chunk_id]])).rows[0].count);assert.equal(firstCount,3);await service.retrieve(requirement.id,{top_k:3});assert.equal(calls[1].length,1);const secondCount=Number((await pool.query(`SELECT count(*) FROM material_chunk_embeddings WHERE chunk_id=ANY($1::text[])`,[[relevant.chunk.chunk_id,history.chunk.chunk_id,unrelated.chunk.chunk_id]])).rows[0].count);assert.equal(secondCount,3);
