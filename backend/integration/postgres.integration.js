...
test('Enterprise Retrieval pgvector Top-K、隔离、失效与审计闭环',async()=>{
  ...
  try{
    ...
    const relevant=await addMaterial(project,'project_case','麒麟鲲鹏国产化适配项目','relevant');const history=await addMaterial(project,'historical_bid','历史标书中的国产化响应','history');const unrelated=await addMaterial(project,'company_profile','公司地址和联系方式','unrelated');const cross=await addMaterial(other,'project_case','完全匹配的国产化环境部署','cross');
    ...
    const app=createApp({repository,storage:{},generationService:{},requirementParseService:{},requirementSourceService:{},productionBetaService:{},companyMaterialService:{},evidenceService:{},enterpriseRetrievalService:service,documentGenerationService:{}});server=await new Promise((resolve)=>{const listener=app.listen(0,'127.0.0.1',()=>resolve(listener));});const base=`http://127.0.0.1:${server.address().port}`;let response=await fetch(`${base}/api/requirements/${requirement.id}/enterprise-retrieval`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({top_k:3})});let body=await response.json();assert.equal(response.status,201);assert.deepEqual(body.data.results.map((item)=>item.rank),[1,2,3]);assert.equal(body.data.results[0].chunk_id,relevant.chunk.chunk_id);assert.equal(body.data.results[1].chunk_id,history.chunk.chunk_id);assert.deepEqual(body.data.results[1].risk_flags,['HISTORICAL_BID_REFERENCE_ONLY']);assert.equal(body.data.results.some((item)=>item.chunk_id===cross.chunk.chunk_id),false);assert.equal(body.data.results.every((item)=>item.evidence_created===false),true);const firstRun=body.data.run;
    response=await fetch(`${base}/api/enterprise-retrieval-runs/${firstRun.retrieval_run_id}`);body=await response.json();assert.equal(body.data.run.status,'succeeded');assert.equal(body.data.run.query_text,'国产化环境部署');assert.equal(body.data.run.top_k,3);assert.ok(body.data.run.completed_at);assert.equal(body.data.results.length,3);
    const evidenceService=new EvidenceService({repository});const evidence=await evidenceService.create(project.id,{material_id:relevant.material.id,source_chunk_id:relevant.chunk.chunk_id,evidence_type:'project_case',title:'国产化项目依据',content:'仅作为相关参考'});await evidenceService.setValidity(evidence.id,{validity_status:'active',reviewed_by:'integration'});await evidenceService.decide(evidence.id,'approved',{decided_by:'integration'});
    const proposed=await evidenceService.proposeMapping(project.id,{requirement_id:'REQ-001',evidence_id:evidence.id,mapping_source:'retrieval',support_level:'reference_only',review_notes:'语义相关不等于完整事实支撑。',retrieval_run_id:firstRun.retrieval_run_id,retrieval_chunk_id:relevant.chunk.chunk_id,created_by:'integration'});assert.equal(proposed.mapping_status,'proposed');
    ...
    const otherRun=(await service.retrieve(otherRequirement.id,{top_k:1})).run;await assert.rejects(()=>evidenceService.proposeMapping(project.id,{requirement_id:'REQ-001',evidence_id:evidence.id,mapping_source:'retrieval',support_level:'partial_support',retrieval_run_id:otherRun.retrieval_run_id,retrieval_chunk_id:cross.chunk.chunk_id,created_by:'integration'}),(error)=>error.code==='EVIDENCE_RETRIEVAL_PROVENANCE_INVALID');
    const approved=await evidenceService.decideMapping(proposed.mapping_id,'approved',{reviewed_by:'integration'});assert.equal(approved.support_level,'reference_only');let formal=await evidenceService.listApprovedForRequirement(project.id,'REQ-001');assert.equal(formal.evidences.length,1);assert.equal(formal.evidences[0].usable_for_claims,false);await evidenceService.decideMapping(proposed.mapping_id,'rejected',{reviewed_by:'integration'});formal=await evidenceService.listApprovedForRequirement(project.id,'REQ-001');assert.equal(formal.evidences.length,0);
    ...
    const firstCount=Number((await pool.query(`SELECT count(*) FROM material_chunk_embeddings WHERE chunk_id=ANY($1::text[])`,[[relevant.chunk.chunk_id,history.chunk.chunk_id,unrelated.chunk.chunk_id]])).rows[0].count);assert.equal(firstCount,3);await service.retrieve(requirement.id,{top_k:3});assert.equal(calls[2].length,1);const secondCount=Number((await pool.query(`SELECT count(*) FROM material_chunk_embeddings WHERE chunk_id=ANY($1::text[])`,[[relevant.chunk.chunk_id,history.chunk.chunk_id,unrelated.chunk.chunk_id]])).rows[0].count);assert.equal(secondCount,3);
    const changedText='麒麟国产化新版适配';const changedHash=createHash('sha256').update(changedText).digest('hex');await pool.query(`UPDATE material_chunks SET source_text=$2,chunk_hash=$3,char_end=char_start+$4 WHERE chunk_id=$1`,[relevant.chunk.chunk_id,changedText,changedHash,changedText.length]);const changed=await service.retrieve(requirement.id,{top_k:3});assert.equal(changed.results[0].chunk_hash,changedHash);assert.equal(Number((await pool.query(`SELECT count(*) FROM material_chunk_embeddings WHERE chunk_id=$1`,[relevant.chunk.chunk_id])).rows[0].count),2);
