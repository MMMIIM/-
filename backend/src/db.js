...
export class PgRepository {
  ...
  async getEvidenceReviewCandidate({projectId,requirementId,retrievalRunId,retrievalCandidateId,sourceSpanId}){const{rows}=await this.pool.query(`SELECT $1::uuid AS project_id,r.id AS requirement_db_id,r.req_id AS requirement_id,r.content AS requirement_text,rr.retrieval_run_id,rr.chunk_id AS retrieval_candidate_id,rr.content_role,rr.chunk_role,rr.candidate_eligibility,rr.substantive_candidate,rr.substantive_class,rr.substantive_reason,rr.role_compatibility,rr.matched_evidence_needs,s.span_id AS source_span_id,s.source_text,s.source_text_hash,s.resolver_version,m.id AS material_id,m.original_name AS source_material,m.material_type FROM requirements r JOIN enterprise_retrieval_runs er ON er.requirement_id=r.id AND er.retrieval_run_id=$3 AND er.status='succeeded' JOIN enterprise_retrieval_results rr ON rr.retrieval_run_id=er.retrieval_run_id AND rr.chunk_id=$4 JOIN evidence_source_spans s ON s.span_id=$5 AND s.project_id=$1 AND s.anchor_chunk_id=rr.chunk_id JOIN company_materials m ON m.id=s.material_id AND m.project_id=$1 WHERE r.project_id=$1 AND r.req_id=$2`,[projectId,requirementId,retrievalRunId,retrievalCandidateId,sourceSpanId]);return rows[0]||null;}
