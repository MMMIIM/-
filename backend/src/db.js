...
export class PgRepository {
  ...
  async listChunksForRetrieval({projectId,materialTypes,materialIds=[],model,version}){const{rows}=await this.pool.query(`SELECT c.*,m.project_id,m.material_type,e.embedding_id,e.embedding_dimension FROM material_chunks c JOIN company_materials m ON m.id=c.material_id LEFT JOIN material_chunk_embeddings e ON e.chunk_id=c.chunk_id AND e.chunk_hash=c.chunk_hash AND e.embedding_model=$3 AND e.embedding_version=$4 WHERE m.project_id=$1 AND m.extraction_status='succeeded' AND (cardinality($2::text[])=0 OR m.material_type=ANY($2::text[])) AND (cardinality($5::uuid[])=0 OR m.id=ANY($5::uuid[])) ORDER BY c.material_id,c.chunk_index`,[projectId,materialTypes,model,version,materialIds]);return rows;}
  async prepareRetrievalCandidates({queryVector,newEmbeddings,projectId,materialTypes,materialIds=[],model,version,dimension,candidateK}){
    const vector=(values)=>`[${values.join(',')}]`;const client=await this.pool.connect();try{await client.query('BEGIN');
      ...
      const{rows}=await client.query(`SELECT e.embedding_id,c.chunk_id,c.material_id,m.material_type,c.source_text,c.chunk_hash,e.embedding_model,e.embedding_version,1-(e.embedding <=> $1::vector) AS similarity_score FROM material_chunk_embeddings e JOIN material_chunks c ON c.chunk_id=e.chunk_id AND c.chunk_hash=e.chunk_hash JOIN company_materials m ON m.id=c.material_id WHERE m.project_id=$2 AND m.extraction_status='succeeded' AND e.embedding_model=$3 AND e.embedding_version=$4 AND e.embedding_dimension=$5 AND (cardinality($6::text[])=0 OR m.material_type=ANY($6::text[])) AND (cardinality($7::uuid[])=0 OR m.id=ANY($7::uuid[])) ORDER BY e.embedding <=> $1::vector,e.embedding_id LIMIT $8`,[vector(queryVector),projectId,model,version,dimension,materialTypes,materialIds,candidateK]);await client.query('COMMIT');return rows.map((row,index)=>({...row,similarity_score:Number(row.similarity_score),rank:index+1,raw_vector_rank:index+1,raw_similarity:Number(row.similarity_score)}));
    }catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}
  }
