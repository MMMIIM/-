...
export class PgRepository {
  ...
  async completeRetrievalRun({runId,queryVector,newEmbeddings,projectId,materialTypes,model,version,dimension,topK,latencyMs}){
    const vector=(values)=>`[${values.join(',')}]`;const client=await this.pool.connect();try{await client.query('BEGIN');
      ...
      const{rows}=await client.query(`SELECT e.embedding_id,c.chunk_id,c.material_id,m.material_type,c.source_text,c.chunk_hash,e.embedding_model,e.embedding_version,1-(e.embedding <=> $1::vector) AS similarity_score FROM material_chunk_embeddings e JOIN material_chunks c ON c.chunk_id=e.chunk_id AND c.chunk_hash=e.chunk_hash JOIN company_materials m ON m.id=c.material_id WHERE m.project_id=$2 AND m.extraction_status='succeeded' AND e.embedding_model=$3 AND e.embedding_version=$4 AND e.embedding_dimension=$5 AND (cardinality($6::text[])=0 OR m.material_type=ANY($6::text[])) ORDER BY e.embedding <=> $1::vector,e.embedding_id LIMIT $7`,[vector(queryVector),projectId,model,version,dimension,materialTypes,topK]);
      for(let index=0;index<rows.length;index+=1)await client.query(`INSERT INTO enterprise_retrieval_results(retrieval_run_id,chunk_id,embedding_id,rank,similarity_score) VALUES($1,$2,$3,$4,$5)`,[runId,rows[index].chunk_id,rows[index].embedding_id,index+1,rows[index].similarity_score]);
      const run=(await client.query(`UPDATE enterprise_retrieval_runs SET status='succeeded',completed_at=now(),latency_ms=$2 WHERE retrieval_run_id=$1 RETURNING *`,[runId,latencyMs])).rows[0];await client.query('COMMIT');return{run,results:rows.map((row,index)=>({...row,similarity_score:Number(row.similarity_score),rank:index+1,risk_flags:row.material_type==='historical_bid'?['HISTORICAL_BID_REFERENCE_ONLY']:[],evidence_created:false}))};
    }catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}
  }
  ...
  async getRetrievalRun(runId){const run=(await this.pool.query(`SELECT * FROM enterprise_retrieval_runs WHERE retrieval_run_id=$1`,[runId])).rows[0];if(!run)return null;const{rows}=await this.pool.query(`SELECT rr.rank,rr.similarity_score,c.chunk_id,c.material_id,c.source_text,c.chunk_hash,m.material_type,e.embedding_model,e.embedding_version FROM enterprise_retrieval_results rr JOIN material_chunks c ON c.chunk_id=rr.chunk_id JOIN company_materials m ON m.id=c.material_id JOIN material_chunk_embeddings e ON e.embedding_id=rr.embedding_id WHERE rr.retrieval_run_id=$1 ORDER BY rr.rank`,[runId]);return{run,results:rows.map((row)=>({...row,similarity_score:Number(row.similarity_score),risk_flags:row.material_type==='historical_bid'?['HISTORICAL_BID_REFERENCE_ONLY']:[],evidence_created:false}))};}
  ...
  async findEvidenceBySourceChunk(projectId,chunkId){const{rows}=await this.pool.query(`SELECT * FROM evidences WHERE project_id=$1 AND source_chunk_id=$2 AND evidence_origin='enterprise' ORDER BY created_at LIMIT 1`,[projectId,chunkId]);return rows[0]||null;}
  ...
  async listEnterpriseEvidenceBindings(projectId){const{rows}=await this.pool.query(`SELECT $1::uuid AS project_id,r.req_id AS requirement_id,rem.mapping_id,rem.mapping_status,rem.support_level,rem.review_notes,e.evidence_id,e.approval_status,e.validity_status,e.evidence_scope,e.capability_tags,e.metadata,e.content,e.source_text,e.title,e.evidence_type,m.material_type,(mc.chunk_id IS NOT NULL AND mc.material_id=e.source_document_id AND mc.source_text=e.source_text AND mc.chunk_hash=e.source_hash) AS source_lineage_verified,(e.approval_status='approved' AND e.validity_status NOT IN ('expired','revoked') AND (e.metadata->>'valid_until' IS NULL OR e.metadata->>'valid_until'>=CURRENT_DATE::text) AND mc.chunk_id IS NOT NULL AND mc.material_id=e.source_document_id AND mc.source_text=e.source_text AND mc.chunk_hash=e.source_hash AND m.material_type<>'historical_bid') AS usable_for_claims FROM requirement_evidence_mappings rem JOIN requirements r ON r.id=rem.requirement_id AND r.project_id=$1 JOIN evidences e ON e.id=rem.evidence_id AND e.project_id=$1 AND e.evidence_origin='enterprise' LEFT JOIN company_materials m ON m.id=e.source_document_id LEFT JOIN material_chunks mc ON mc.chunk_id=e.source_chunk_id ORDER BY r.ordinal,rem.created_at,rem.mapping_id`,[projectId]);return rows;}
  ...
  async validateEvidenceForMapping(projectId,evidenceId){const{rows}=await this.pool.query(`SELECT e.approval_status,(mc.chunk_id IS NOT NULL AND mc.material_id=e.source_document_id AND mc.source_text=e.source_text AND mc.chunk_hash=e.source_hash) AS source_lineage_verified FROM evidences e LEFT JOIN material_chunks mc ON mc.chunk_id=e.source_chunk_id WHERE e.id=$2 AND e.project_id=$1 AND e.evidence_origin='enterprise'`,[projectId,evidenceId]);return rows[0]||null;}
  ...
  async getEvidenceFactByIdentifier(identifier){const{rows}=await this.pool.query(`SELECT ef.*,e.approval_status AS evidence_approval_status,e.validity_status AS evidence_validity_status,m.material_type,(mc.chunk_id IS NOT NULL AND mc.material_id=e.source_document_id AND mc.source_text=e.source_text AND mc.chunk_hash=e.source_hash) AS source_lineage_verified FROM evidence_facts ef JOIN evidences e ON e.id=ef.evidence_id LEFT JOIN company_materials m ON m.id=e.source_document_id LEFT JOIN material_chunks mc ON mc.chunk_id=e.source_chunk_id WHERE ef.fact_id=$1 OR ef.id::text=$1`,[identifier]);return rows[0]||null;}
