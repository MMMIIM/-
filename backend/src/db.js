...
export class PgRepository {
  ...
  async getEvidenceReviewForFact({projectId,reviewId}){const{rows}=await this.pool.query(`SELECT ecr.review_id,ecr.project_id,ecr.review_status,ecr.source_span_id,ecr.source_text_hash,ecr.evidence_capability,ecr.support_level,ecr.contract_version AS evidence_review_contract_version,s.material_id,s.anchor_chunk_id,s.source_text,s.source_text_hash AS current_source_text_hash,m.material_type FROM evidence_candidate_reviews ecr JOIN evidence_source_spans s ON s.span_id=ecr.source_span_id AND s.project_id=ecr.project_id JOIN company_materials m ON m.id=s.material_id AND m.project_id=ecr.project_id WHERE ecr.project_id=$1 AND ecr.review_id=$2`,[projectId,reviewId]);return rows[0]||null;}
  ...
  async listChunksForRetrieval({projectId,materialTypes,materialIds=[],model,version,corpusScopes=['GENERAL'],corpusProjectId=PUBLIC_CORPUS_PROJECT_ID}){const{rows}=await this.pool.query(`SELECT c.*,m.project_id,m.material_type,m.corpus_scope,m.industry,m.original_name,e.embedding_id,e.embedding_dimension FROM material_chunks c JOIN company_materials m ON m.id=c.material_id LEFT JOIN material_chunk_embeddings e ON e.chunk_id=c.chunk_id AND e.chunk_hash=c.chunk_hash AND e.embedding_model=$3 AND e.embedding_version=$4 WHERE m.extraction_status='succeeded' AND ((m.project_id=$1 AND m.corpus_scope='ENTERPRISE_PRIVATE') OR (m.project_id=$5 AND m.lifecycle_status='ACTIVE' AND m.review_status='approved' AND m.usage_status=ANY(ARRAY['ACTIVE_FULLTEXT','ACTIVE_EXCERPT']) AND m.index_status='INDEXED' AND m.corpus_scope=ANY($6::text[]))) AND (cardinality($2::text[])=0 OR m.material_type=ANY($2::text[])) AND (cardinality($7::uuid[])=0 OR m.id=ANY($7::uuid[])) ORDER BY m.project_id,m.id,c.chunk_index`,[projectId,materialTypes,model,version,corpusProjectId,corpusScopes,materialIds]);return rows;}
  async prepareRetrievalCandidates({queryVector,newEmbeddings,projectId,materialTypes,materialIds=[],model,version,dimension,candidateK,corpusScopes=['GENERAL'],corpusProjectId=PUBLIC_CORPUS_PROJECT_ID}){
    const vector=(values)=>`[${values.join(',')}]`;const client=await this.pool.connect();try{await client.query('BEGIN');
      ...
      const{rows}=await client.query(`SELECT e.embedding_id,c.chunk_id,c.material_id,m.project_id AS project_id,m.material_type,m.corpus_scope,m.industry,m.original_name,m.source_type,m.source_org,m.authority_level AS source_authority,m.lifecycle_status,m.review_status,m.usage_status,m.index_status,m.effective_status,m.synthetic_test_material,c.source_text,c.chunk_hash,e.embedding_model,e.embedding_version,1-(e.embedding <=> $1::vector) AS similarity_score FROM material_chunk_embeddings e JOIN material_chunks c ON c.chunk_id=e.chunk_id AND c.chunk_hash=e.chunk_hash JOIN company_materials m ON m.id=c.material_id WHERE m.extraction_status='succeeded' AND ((m.project_id=$2 AND m.corpus_scope='ENTERPRISE_PRIVATE') OR (m.project_id=$3 AND m.lifecycle_status='ACTIVE' AND m.review_status='approved' AND m.usage_status=ANY(ARRAY['ACTIVE_FULLTEXT','ACTIVE_EXCERPT']) AND m.index_status='INDEXED' AND m.corpus_scope=ANY($4::text[]))) AND e.embedding_model=$5 AND e.embedding_version=$6 AND e.embedding_dimension=$7 AND (cardinality($8::text[])=0 OR m.material_type=ANY($8::text[])) AND (cardinality($9::uuid[])=0 OR m.id=ANY($9::uuid[])) ORDER BY e.embedding <=> $1::vector,e.embedding_id LIMIT $10`,[vector(queryVector),projectId,corpusProjectId,corpusScopes,model,version,dimension,materialTypes,materialIds,candidateK]);await client.query('COMMIT');return rows.map((row,index)=>({...row,similarity_score:Number(row.similarity_score),rank:index+1,raw_vector_rank:index+1,raw_similarity:Number(row.similarity_score)}));
    }catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}
  }
  ...
  async listApprovedCurrentEvidenceFacts(projectId){const{rows}=await this.pool.query(`SELECT ef.*,e.metadata,e.evidence_id AS evidence_identifier,true AS is_current FROM evidence_facts ef JOIN evidences e ON e.id=ef.evidence_id AND e.project_id=ef.project_id WHERE ef.project_id=$1 AND ef.review_status='approved' AND COALESCE(e.metadata->>'lifecycle_role','') <> 'PRE_REVIEW_STAGING' AND NOT EXISTS(SELECT 1 FROM evidence_facts successor WHERE successor.supersedes_fact_id=ef.id AND successor.review_status='approved') ORDER BY ef.evidence_id,ef.created_at,ef.version`,[projectId]);return rows;}
  ...
  async validateEvidenceForMapping(projectId,evidenceId){const{rows}=await this.pool.query(`SELECT e.approval_status,e.metadata,(${EVIDENCE_LINEAGE_SQL}) AS source_lineage_verified FROM evidences e LEFT JOIN company_materials m ON m.id=e.source_document_id LEFT JOIN material_chunks mc ON mc.chunk_id=e.source_chunk_id WHERE e.id=$2 AND e.project_id=$1 AND e.evidence_origin='enterprise'`,[projectId,evidenceId]);return rows[0]||null;}
