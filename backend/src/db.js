...
export class PgRepository {
  ...
  async finishDocumentTask(generationId,batch,status,data){await this.pool.query(`UPDATE document_generation_tasks SET status=$4,output_markdown=$5,attempt=$6,runtime_ms=$7,error_code=$8,error_message=$9,provider_audit=$10,updated_at=now() WHERE generation_id=$1 AND chapter_id=$2 AND batch_index=$3`,[generationId,batch.chapter_id,batch.batch_index,status,data.output_markdown||null,data.attempt||0,data.runtime_ms||null,data.error_code||null,data.error_message||null,JSON.stringify(data.provider_audit||{})]);}
