  ...
  async completeGeneration({ job, parsed, workflowVersion, runtimeMs }) {
    ...
    try {
      ...
      const generationResult = await client.query(`
        INSERT INTO generations (project_id, job_id, response_payload_json, workflow_version, runtime_ms)
        VALUES ($1, $2, $3, $4, $5) RETURNING *
      `, [job.project_id, job.id, parsed.raw, workflowVersion, runtimeMs]);
      let version = null;
      if (job.project_id) {
        const versionResult = await client.query(`
          INSERT INTO document_versions
            (project_id, generation_id, version_number, title, content_markdown, sections_json, warnings_json, risk_status)
          SELECT $1, $2, COALESCE(MAX(version_number), 0) + 1, $3, $4, $5, $6, $7
          FROM document_versions WHERE project_id = $1 RETURNING *
        `, [job.project_id, generationResult.rows[0].id, parsed.title, parsed.markdown, parsed.sections, parsed.warnings, parsed.riskStatus]);
