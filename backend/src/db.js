...
export class PgRepository {
  ...
  async prepareRetrievalCandidates({queryVector,newEmbeddings,projectId,materialTypes,materialIds=[],model,version,dimension,candidateK,corpusScopes=['GENERAL'],corpusProjectId=PUBLIC_CORPUS_PROJECT_ID}){
    const vector=(values)=>`[${values.join(',')}]`;const client=await this.pool.connect();try{await client.query('BEGIN');
      ...
      const{rows}=await client.query(`SELECT e.embedding_id,c.chunk_id,c.material_id,m.material_type,m.corpus_scope,m.industry,m.original_name,c.source_text,c.chunk_hash,e.embedding_model,e.embedding_version,1-(e.embedding <=> $1::vector) AS similarity_score FROM material_chunk_embeddings e JOIN material_chunks c ON c.chunk_id=e.chunk_id AND c.chunk_hash=e.chunk_hash JOIN company_materials m ON m.id=c.material_id WHERE m.extraction_status='succeeded' AND ((m.project_id=$2 AND m.corpus_scope='ENTERPRISE_PRIVATE') OR (m.project_id=$3 AND m.lifecycle_status='ACTIVE' AND m.review_status='approved' AND m.usage_status=ANY(ARRAY['ACTIVE_FULLTEXT','ACTIVE_EXCERPT']) AND m.index_status='INDEXED' AND m.corpus_scope=ANY($4::text[]))) AND e.embedding_model=$5 AND e.embedding_version=$6 AND e.embedding_dimension=$7 AND (cardinality($8::text[])=0 OR m.material_type=ANY($8::text[])) AND (cardinality($9::uuid[])=0 OR m.id=ANY($9::uuid[])) ORDER BY e.embedding <=> $1::vector,e.embedding_id LIMIT $10`,[vector(queryVector),projectId,corpusProjectId,corpusScopes,model,version,dimension,materialTypes,materialIds,candidateK]);await client.query('COMMIT');return rows.map((row,index)=>({...row,similarity_score:Number(row.similarity_score),rank:index+1,raw_vector_rank:index+1,raw_similarity:Number(row.similarity_score)}));
    }catch(error){await client.query('ROLLBACK');throw error;}finally{client.release();}
  }
