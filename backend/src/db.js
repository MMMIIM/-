...
export class PgRepository {
  ...
  async finishExternalWriterCallAudit(id,v){const{rows}=await this.pool.query(`UPDATE external_writer_call_audits SET provider_request_id=$2,status=$3,http_status=$4,input_tokens=$5,output_tokens=$6,latency_ms=$7,error_code=$8,error_message=$9,finished_at=now() WHERE id=$1 RETURNING *`,[id,v.provider_request_id||null,v.status,v.http_status||null,v.input_tokens??null,v.output_tokens??null,v.latency_ms??null,v.error_code||null,v.error_message||null]);return rows[0];}
