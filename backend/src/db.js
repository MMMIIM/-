...
export class PgRepository {
  ...
  async listEvidenceCandidateReviews(projectId){const{rows}=await this.pool.query(`SELECT ecr.*,r.req_id AS requirement_ref,r.content AS requirement_text,r.is_mandatory,r.source_page_start,r.source_page_end,r.source_paragraph_start,r.source_paragraph_end,s.source_text AS source_excerpt,m.original_name AS source_material,m.material_type FROM evidence_candidate_reviews ecr JOIN requirements r ON r.id=ecr.requirement_id JOIN evidence_source_spans s ON s.span_id=ecr.source_span_id JOIN company_materials m ON m.id=s.material_id WHERE ecr.project_id=$1 ORDER BY ecr.created_at,ecr.review_id`,[projectId]);return rows;}
