...
export class PgRepository {
  ...
  async createExternalWriterCallAudit(v){const{rows}=await this.pool.query(`INSERT INTO external_writer_call_audits(writer_task_id,project_id,provider,model,endpoint_host,sanitized_request_hash,status,external_request_count) VALUES($1,$2,$3,$4,$5,$6,'started',1) RETURNING *`,[v.writer_task_id,v.project_id,v.provider,v.model,v.endpoint_host,v.sanitized_request_hash]);return rows[0];}
  async finishExternalWriterCallAudit(id,v){const{rows}=await this.pool.query(`UPDATE external_writer_call_audits SET provider_request_id=$2,status=$3,http_status=$4,input_tokens=$5,output_tokens=$6,latency_ms=$7,error_code=$8,error_message=$9,finish_reason=$10,response_char_count=$11,response_content_hash=$12,response_format_mode=$13,json_parse_success=$14,schema_validation_success=$15,parse_failure_class=$16,parse_error_offset=$17,finished_at=now() WHERE id=$1 RETURNING *`,[id,v.provider_request_id||null,v.status,v.http_status||null,v.input_tokens??null,v.output_tokens??null,v.latency_ms??null,v.error_code||null,v.error_message||null,v.finish_reason||null,v.response_char_count??null,v.response_content_hash||null,v.response_format_mode||null,v.json_parse_success??null,v.schema_validation_success??null,v.parse_failure_class||null,v.parse_error_offset??null]);return rows[0];}
