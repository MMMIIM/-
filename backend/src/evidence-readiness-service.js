...
export class EvidenceReadinessService{
 ...
 async get(projectId){
  ...
  const items=requirements.map(requirement=>{const related=byReq.get(requirement.req_id)||[],relatedReviews=reviewsByReq.get(requirement.req_id)||[],relatedFacts=relatedReviews.flatMap(x=>factsByReview.get(x.review_id)||[]);let readiness=deriveReadiness(related);const lifecyclePending=relatedReviews.some(x=>['proposed','needs_review'].includes(x.review_status))||relatedFacts.some(x=>x.review_status==='draft');if(readiness!=='CONFLICT'&&lifecyclePending)readiness='NEEDS_REVIEW';const approvedFacts=relatedFacts.filter(x=>x.review_status==='approved'),suggested=suggestMaterialCategory(requirement,related);return{requirement_id:requirement.req_id,requirement_text:requirement.text,is_mandatory:requirement.is_mandatory,requirement_category:requirement.requirement_category,source_evidence:requirement.source_evidence,readiness,readiness_label:LABELS[readiness],existing_evidence_count:new Set([...related.map(x=>x.material_id),...relatedReviews.map(x=>x.material_id)].filter(Boolean)).size,approved_fact_count:new Set(approvedFacts.map(x=>x.fact_id)).size,mapping_summary:related.map(x=>({mapping_id:x.mapping_id,review_status:x.review_status,support_level:x.support_level,semantic_relationship:x.semantic_relationship,evidence_fact_id:x.evidence_fact_id,source:{evidence_review_id:x.source?.evidence_review_id||null,material_id:x.material_id,material_name:x.material_name,material_type:x.material_type,source_span_id:x.source_span_id}})),gap_reason:this.gapReason(readiness),suggested_material_category:suggested?.category||null,suggested_material:suggested?.suggestion||null};});
  ...
  return{formula:'SUPPORTED / TOTAL_REQUIREMENTS',summary:{total_requirements:total,supported:counts.SUPPORTED,partially_supported:counts.PARTIALLY_SUPPORTED,insufficient:counts.INSUFFICIENT,conflict:counts.CONFLICT,no_evidence:counts.NO_EVIDENCE,needs_review:counts.NEEDS_REVIEW,readiness_rate:total?counts.SUPPORTED/total:0},requirements:items,gaps,suggested_materials};
 }
