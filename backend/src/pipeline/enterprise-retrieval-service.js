...
export class EnterpriseRetrievalService{
  ...
  async retrieve(requirementId,input={}){
    ...
    const legacyLimit=topK(input.top_k,this.defaultTopK);const materialTypes=[...new Set((input.material_types||[]).map(String))];if(materialTypes.some((type)=>!TYPES.has(type)))throw new AppError('RETRIEVAL_FILTER_INVALID','material_types 包含无效值。',422);
    ...
    try{
      ...
      const rawCandidates=await this.repository.prepareRetrievalCandidates({queryVector,newEmbeddings,projectId:requirement.project_id,materialTypes,...config,candidateK:PRODUCTION_CANDIDATE_K});const ranking=rerankProductionCandidates(rawCandidates,semanticMetadata);const selected=ranking.final_candidates.slice(0,Math.min(PRODUCTION_REVIEW_K,legacyLimit));
      return await this.repository.completeRetrievalRun({runId:run.retrieval_run_id,ranking:{...ranking,final_candidates:selected},latencyMs:Math.max(0,this.clock()-started),...config});
