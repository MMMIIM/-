...
export class EnterpriseRetrievalService{
  ...
  async retrieve(requirementId,input={}){
    ...
    try{
      ...
      const rawCandidates=await this.repository.prepareRetrievalCandidates({queryVector,newEmbeddings,projectId:requirement.project_id,materialTypes,materialIds:selectedMaterialIds,corpusScopes,corpusProjectId:PUBLIC_CORPUS_PROJECT_ID,...config,candidateK:PRODUCTION_CANDIDATE_K});const ranking=rerankProductionCandidates(rawCandidates,semanticMetadata);const selected=ranking.final_candidates.slice(0,Math.min(PRODUCTION_REVIEW_K,legacyLimit));
