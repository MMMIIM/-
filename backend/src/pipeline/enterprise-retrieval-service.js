...
export class EnterpriseRetrievalService{
  ...
  async retrieve(requirementId,input={}){
    ...
    const legacyLimit=topK(input.top_k,this.defaultTopK);const materialTypes=[...new Set((input.material_types||[]).map(String))];if(materialTypes.some((type)=>!TYPES.has(type)))throw new AppError('RETRIEVAL_FILTER_INVALID','material_types 包含无效值。',422);const selectedMaterialIds=materialIds(input.material_ids);
    const semanticMetadata=input.semantic_metadata??{};const config={model:this.embeddingClient.model,version:this.embeddingClient.version,dimension:this.embeddingClient.dimension};const filters={project_id:requirement.project_id,extraction_status:'succeeded',material_types:materialTypes,material_ids:selectedMaterialIds};const started=this.clock();const run=await this.repository.createRetrievalRun({projectId:requirement.project_id,requirementDbId:requirement.id,requirementRef:requirement.req_id,queryText:requirement.text,queryHash:sha(requirement.text),...config,topK:legacyLimit,candidateK:PRODUCTION_CANDIDATE_K,reviewK:Math.min(PRODUCTION_REVIEW_K,legacyLimit),filters,semanticMetadata,retrievalContractVersion:RETRIEVAL_CONTRACT_VERSION,rerankVersion:RERANK_VERSION});
    try{
      const chunks=await this.repository.listChunksForRetrieval({projectId:requirement.project_id,materialTypes,materialIds:selectedMaterialIds,...config});if(chunks.some((item)=>item.embedding_id&&Number(item.embedding_dimension)!==config.dimension))throw new AppError('EMBEDDING_IDENTITY_CONFLICT','同一 embedding model/version 的维度不一致，请更新 embedding_version。',409);const missing=chunks.filter((item)=>!item.embedding_id);const vectors=await this.embeddingClient.embed([requirement.text,...missing.map((item)=>item.source_text)]);const queryVector=vectors[0];const newEmbeddings=missing.map((chunk,index)=>({chunkId:chunk.chunk_id,chunkHash:chunk.chunk_hash,embedding:vectors[index+1],...config}));
      const rawCandidates=await this.repository.prepareRetrievalCandidates({queryVector,newEmbeddings,projectId:requirement.project_id,materialTypes,materialIds:selectedMaterialIds,...config,candidateK:PRODUCTION_CANDIDATE_K});const ranking=rerankProductionCandidates(rawCandidates,semanticMetadata);const selected=ranking.final_candidates.slice(0,Math.min(PRODUCTION_REVIEW_K,legacyLimit));
      const result=await this.repository.completeRetrievalRun({runId:run.retrieval_run_id,ranking:{...ranking,final_candidates:selected},latencyMs:Math.max(0,this.clock()-started),...config});return{...result,answer_status:selected.length?'CANDIDATES_FOUND':'NO_RELEVANT_EVIDENCE',no_answer_code:selected.length?null:'NO_RELEVANT_EVIDENCE'};
