...
const EXPECTED_CLASSIFICATIONS = Object.freeze({
  ...
  'V2R-006-ISO-SCOPE': ['TOPIC_RELEVANT_ONLY', 'METADATA_OR_HEADER', 'TOPIC_RELEVANT_ONLY', 'METADATA_OR_HEADER', 'TOPIC_RELEVANT_ONLY'],
...
const EXPECTED_REASONS = Object.freeze({
  ...
  'V2R-006-ISO-SCOPE': ['enterprise certificate lacks specified project-subject scope', 'title-only metadata', 'ISO 27001 facts still lack project-subject scope', 'wrong certificate title-only metadata', 'ISO 9001 is the wrong certificate and scope'],
...
function auditGoldBinding(mapping, corpus) {
  ...
  const lineageValid = Object.values(checks).every(Boolean);
  ...
  return {
    ...
    classification: lineageValid ? 'GOLD_BINDING_VALID' : 'GOLD_LINEAGE_INVALID',
    checks,
    expected: {
    ...
    repaired_binding: repairedChunk ? {
      status: span && repairedChunk.material_id === mapping.expected_material_id && text(span.source_text).includes(text(repairedChunk.source_text)) ? 'GOLD_BINDING_VALID' : 'GOLD_LINEAGE_INVALID',
      material_id: mapping.expected_material_id,
      document_id: mapping.expected_document_id,
...
function auditSummary(candidateAudit, goldAudits) {
  ...
  const persistedValidBindings = goldAudits.filter(item => item.classification === 'GOLD_BINDING_VALID').length;
  const repairedValidBindings = goldAudits.filter(item => item.repaired_binding?.status === 'GOLD_BINDING_VALID').length;
  return {
...
function offlineMetrics(livePacket, candidateAudit, goldAudits) {
  const decisionCases = ['V2R-001-PERF-DIRECT', 'V2R-002-PERF-PARTIAL', 'V2R-003-COMP-DIRECT', 'V2R-004-COMP-PARTIAL', 'V2R-005-ISO-DIRECT'];
  ...
  const caseMetrics = decisionCases.map(caseId => {
    ...
    const expectedChunk = gold?.repaired_binding?.status === 'GOLD_BINDING_VALID'
  ...
  return {
    ...
    excluded_from_decision_metrics: ['V2R-006-ISO-SCOPE (BOUNDARY / subject-scope insufficient)', 'V2R-007-PROJECT-STATUS (GOLD_DESIGN_AMBIGUOUS; equivalent candidate needs review)']
  };
}
