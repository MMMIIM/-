...
test('Retrieval Result 通过真实 Chunk 创建 draft Evidence 且重复请求返回已有记录',async()=>{
  const source='公开材料明确记录的事实。';const [chunk]=chunkEnterpriseMaterial(MATERIAL_ID,source);let existing=null;const material={id:MATERIAL_ID,project_id:PROJECT_ID,material_type:'project_case',extraction_status:'succeeded',extracted_text:source};const repository={getRetrievalEvidenceSource:async()=>({status:'succeeded',material_id:MATERIAL_ID,chunk_id:chunk.chunk_id,material_type:'project_case',original_name:'公开公告',...chunk}),findEvidenceBySourceSpan:async()=>existing,getCompanyMaterial:async()=>material,listMaterialChunks:async()=>[chunk],getMaterialChunk:async()=>chunk,findInvalidConfirmedRequirementIds:async()=>[],createEvidenceRecord:async(value)=>{existing={id:MATERIAL_ID,approval_status:'draft',source_text:value.sourceText,source_hash:value.sourceHash};return existing;}};const service=new EvidenceService({repository});const first=await service.createFromRetrieval(PROJECT_ID,'REQ-030',{retrieval_run_id:PROJECT_ID,chunk_id:chunk.chunk_id,title:'中标事实',evidence_scope:['award_fact']});assert.equal(first.created,true);assert.equal(first.evidence.approval_status,'draft');const second=await service.createFromRetrieval(PROJECT_ID,'REQ-030',{retrieval_run_id:PROJECT_ID,chunk_id:chunk.chunk_id,title:'重复'});assert.equal(second.created,false);assert.equal(second.evidence.id,first.evidence.id);
