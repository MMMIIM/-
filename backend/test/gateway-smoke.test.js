...
test('smoke:gateway 使用最小 requirement_extraction payload 并验证新契约', async () => {
  let sentInputs;const envelope={schema_version:'4.3-requirement-extraction',task_type:'requirement_extraction',status:'success',data:{requirements:[]},warnings:[]};
  ...
  assert.deepEqual(JSON.parse(stdout[0]),{schema_version:'4.3-requirement-extraction',task_type:'requirement_extraction',status:'success',requirements_count:0,warnings_count:0,elapsed_ms:25});
});
