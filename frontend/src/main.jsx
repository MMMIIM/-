...
export function ProductionBeta({ projectId, baseline }) {
  ...
  return <div className="document-layout"><section className="card">
    <div className="section-heading"><div><h2>ResponsePlan → Claim → Coverage</h2><p>系统根据已确认需求建立保守响应骨架，可由用户补充实施动作、条件和证据。基础声明由需求原文确定性生成，只有已批准声明可进入正文。</p></div><button className="primary-inline" disabled={!confirmed||state.running} onClick={run}>{state.running?'建立中…':'建立响应计划'}</button></div>
