...
function Workspace({ projectId, onBack }) {
  ...
  if (activeTab === '材料准备度') return <PageShell title={data.project.name} subtitle="项目工作台 · Evidence Readiness" onBack={onBack}><nav className="tabs">{tabs.map((tab) => <button className={activeTab === tab ? 'active' : ''} key={tab} onClick={() => setActiveTab(tab)}>{tab}</button>)}</nav><EvidenceReadiness projectId={projectId} onOpenReview={()=>setActiveTab('审核中心')} /></PageShell>;
