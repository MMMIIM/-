...
const tabs = ['概览', '材料准备度', '审核中心', '招标文件', '需求解析', '响应规划', '企业材料', '企业证据复核', '标书', '风险复核', '版本记录'];
...
function Workspace({ projectId, onBack }) {
  ...
  if (activeTab === '材料准备度') return <PageShell title={data.project.name} subtitle="项目工作台 · Evidence Readiness" onBack={onBack}><nav className="tabs">{tabs.map((tab) => <button className={activeTab === tab ? 'active' : ''} key={tab} onClick={() => setActiveTab(tab)}>{tab}</button>)}</nav><EvidenceReadiness projectId={projectId} onOpenReview={()=>setActiveTab('审核中心')} onSupplementMaterial={()=>setActiveTab('企业材料')} /></PageShell>;
  if (activeTab === '审核中心') return <PageShell title={data.project.name} subtitle="项目工作台 · 审核与控制中心" onBack={onBack}><nav className="tabs">{tabs.map((tab) => <button className={activeTab === tab ? 'active' : ''} key={tab} onClick={() => setActiveTab(tab)}>{tab}</button>)}</nav><ReviewWorkbench projectId={projectId} /></PageShell>;
