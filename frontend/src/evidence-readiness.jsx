...
const LABEL = { SUPPORTED:'充分支持', PARTIALLY_SUPPORTED:'部分支持', INSUFFICIENT:'证据不足', CONFLICT:'冲突', NO_EVIDENCE:'无证据', NEEDS_REVIEW:'待确认' };
...
export function EvidenceReadiness({ projectId, onOpenReview, onSupplementMaterial, initialData=null }) {
  ...
  return <div className="readiness-layout">
    <section className="card">
      <div className="section-heading"><div><h2>Evidence Readiness / 材料准备度</h2><p>准备度仅聚合正式 Requirement、Evidence Fact 与 Mapping，不代表 Claim 可以安全表达。</p></div><button className="secondary-button" onClick={onOpenReview}>进入审核中心</button></div>
      <div className="readiness-summary"><strong>{Math.round(d.summary.readiness_rate*100)}%</strong><span>充分支持率（SUPPORTED / 全部 Requirement）</span><p>共 {d.summary.total_requirements} 条：充分 {d.summary.supported} · 部分 {d.summary.partially_supported} · 不足 {d.summary.insufficient} · 冲突 {d.summary.conflict} · 无证据 {d.summary.no_evidence} · 待确认 {d.summary.needs_review}</p>{d.generation_readiness?<div className={`notice ${d.generation_readiness.status==='READY_TO_GENERATE'?'success':'warning'}`}><strong>生成准备状态：</strong>{d.generation_readiness.message}</div>:null}</div>
      ...
      <div className="table-scroll"><table className="data-table requirement-table"><thead><tr><th>Requirement</th><th>等级</th><th>准备度</th><th>Evidence / Fact</th><th>缺口摘要</th><th>操作</th></tr></thead><tbody>{visible.map(item=><tr key={item.requirement_id}><td><strong>{item.requirement_id}</strong><small>{item.requirement_text}</small></td><td>{item.is_mandatory?'★ 实质性要求':'一般要求'}</td><td><span className={`badge ${item.readiness}`}>{item.readiness_label}</span></td><td>{item.existing_evidence_count} / {item.approved_fact_count}</td><td>{item.gap_reason||'当前已有充分正式支撑。'}</td><td><button onClick={()=>setSelected(item)}>查看证据与来源</button>{item.readiness==='NEEDS_REVIEW'||item.readiness==='CONFLICT'?<button onClick={onOpenReview}>去审核</button>:null}</td></tr>)}</tbody></table></div>
      {selected?<div className="readiness-detail"><h3>{selected.requirement_id} 规范来源链</h3><p>{selected.requirement_text}</p>{selected.mapping_summary.length?selected.mapping_summary.map(m=><div key={m.mapping_id}><strong>{m.support_level} · {m.review_status}</strong><p>Mapping {m.mapping_id} → Fact {m.evidence_fact_id} → Source Span {m.source.source_span_id}</p><small>{m.source.material_name||'未知材料'} · {m.source.material_type||'未知类型'}</small></div>):<p>当前没有正式 Mapping。</p>}<button onClick={()=>setSelected(null)}>关闭</button></div>:null}
    </section>
