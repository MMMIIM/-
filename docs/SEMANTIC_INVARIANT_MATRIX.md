...
| ID | Invariant | Owner | Production entry point | Service | Entry | Persistence | Negative control | Authorization | Status |
|---|---|---|---|---|---|---|---|---|---|
| I01 | `UNKNOWN != MATCH` | EvidenceSupportAssessment / downstream gates | Review proposal boundary | YES | PARTIAL for transient proposal path | N/A (assessment) / Review persisted | YES | N/A | PARTIAL |
| I02 | `UNKNOWN != MISMATCH` | EvidenceSupportAssessment | Review proposal boundary | YES | PARTIAL for transient proposal path | N/A / Review persisted | YES | N/A | PARTIAL |
| I03 | Required + Unknown → unresolved required | Aggregator / Readiness | Evidence review/readiness routes | YES | YES | Readiness persisted | YES | N/A | ENFORCED |
| I04 | Required + Mismatch cannot be review-ready | Aggregator / Evidence Review | Evidence review decision | YES | YES | Review persisted | YES | N/A | ENFORCED |
| I05 | Blocking mismatch cannot be full support | Assessment / Mapping / Claim Gate | Mapping and Claim routes | YES | YES | Mapping/Claim persisted | YES | N/A | ENFORCED |
| I06 | Blocking conflict → `CONFLICTING_EVIDENCE` | Aggregator | Review proposal boundary | YES | PARTIAL transient entry | N/A / Review downstream | YES | N/A | PARTIAL |
| I07 | Technical unavailable → `ASSESSMENT_UNAVAILABLE` | Gateway evaluator | Provider adapter / review proposal | YES | PARTIAL live provider | N/A / audit metadata only | YES | N/A | PARTIAL |
| I08 | `SYSTEM_DERIVED` is not source evidence | Source eligibility / Eval | Retrieval final-candidate entry | YES | YES | Candidate audit persisted | YES | N/A | ENFORCED |
| I09 | Gold Context is not Exact Gold Hit | Eval owner | Eval runner | YES | YES | Eval artifacts only | YES | N/A | ENFORCED |
...
| I17 | DocumentVersion confirmation uses owning service | Generation/Version service | Version confirmation route | YES | YES | Review decision/version | YES | PARTIAL | ENFORCED |
| I18 | Client actor cannot become formal audit actor | Actor resolver / services | All formal mutation routes | YES | YES | Actor columns | YES | PARTIAL | ENFORCED |
| I19 | Legacy compatibility cannot weaken canonical contract | Compatibility boundary | Legacy routes | YES | PARTIAL for reachable write-capable paths | Formal state as applicable | YES for covered routes | PARTIAL | PARTIAL |
| I20 | Eval/test fixture cannot mutate production truth | Eval boundary | Eval/test runners | YES | YES | N/A or eval artifacts | YES | N/A | ENFORCED |
...
`Service`, `Entry`, `Persistence` or `Negative control`. For security-sensitive
formal-state invariants, `PARTIAL` means the kernel exists but the business
entry point has not adopted it yet. `LIFECYCLE_ENFORCED` is not equivalent to
